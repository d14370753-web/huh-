...
// 統一的點擊運算處理
function unifiedPress(inputX, inputY) {
  let vMouseX = (inputX - offsetX) / currentScale;
  let vMouseY = (inputY - offsetY) / currentScale;
...
// 電腦滑鼠點擊（過濾掉手機觸控引起的模擬點擊）
function mousePressed() {
  if (touches.length === 0) {
    unifiedPress(mouseX, mouseY);
  }
}
...
function touchStarted() {
  if (touches.length > 0) {
    unifiedPress(touches[0].x, touches[0].y);
  }
  return false; // 【超重要】阻止手機瀏覽器預設的網頁滾動、雙擊放大與縮放行為！
}
